...
function renderCards() {
  ...
  list.forEach((car, i) => {
    ...
    const primaryTag = car.tags && car.tags.length > 0 ? car.tags[0].toUpperCase() : 'CAR';

    card.innerHTML = `
      <div class="card-img-wrap">
        <img src="${imgSrc}" alt="${escapeHTML(car.name)}" loading="lazy" onerror="this.onerror=null;this.src='https://via.placeholder.com/600x400?text=3FTYWHLS+Vehicle'" />
        <div class="card-img-overlay"></div>
        <div class="card-tag">${escapeHTML(primaryTag)}</div>
        <div class="card-img-count">&#128247; ${car.images ? car.images.length : 0}</div>
      </div>
      <div class="card-body">
        <div class="card-name">${escapeHTML(car.name)}</div>
        <div class="card-subtitle">${escapeHTML(car.subtitle || '')}</div>
        <div class="card-price">
          ${escapeHTML(car.price)}
          ${car.negotiable ? '<span class="neg">neg.</span>' : ''}
        </div>
        <div class="card-specs">
          ${(car.specs || []).slice(0, 3).map(s => `<span class="spec-chip">${escapeHTML(s.value)}</span>`).join('')}
        </div>
        <div class="card-footer">
          <button class="card-btn card-btn-details" data-idx="${origIdx}" id="details-btn-${car.id}">View Details</button>
          <a class="card-btn card-btn-wa" href="${waLink(car.name)}" target="_blank" rel="noopener noreferrer" id="wa-btn-${car.id}"
...
function openModal(idx) {
  ...
  specsEl.innerHTML = (car.specs || []).map(s => `
    <div class="spec-row">
      <div class="sl">${escapeHTML(s.label)}</div>
      <div class="sv">${escapeHTML(s.value)}</div>
    </div>`).join('');

  // Features
  const featEl = document.getElementById('modal-features');
  featEl.innerHTML = (car.features || []).map(f => `<span class="feat-tag">${escapeHTML(f)}</span>`).join('');
